...
社区通常推荐视频模型使用 32 的倍数作为宽高。未指定分辨率时，本 skill 默认使用 `640×352`；指定分辨率时按用户设置执行。

| 用途/比例 | 尺寸 | MP | H3 使用建议 |
|---|---:|---:|---|
| 横屏 16:9（低显存快速档） | `640×352` | 0.225 | 8 GB 显存首选 |
| 横屏 16:9（实验档） | `736×416` | 0.306 | 提升细节和动作遵循度 |
| 横屏 16:9（约 0.4MP） | `864×480` | 0.415 | 质量更好但更慢 |
| 横屏 16:9（高分辨率） | `1280×704` | 0.901 | 接近 720P，8 GB 显存不建议直接生成 |
| 竖屏 9:16（低显存） | `352×640` | 0.225 | 适合短视频竖屏草稿 |
| 竖屏 9:16（中等质量） | `416×736` | 0.306 | 兼顾细节和显存 |
| 竖屏 9:16（约 0.4MP） | `480×864` | 0.415 | 更清晰但耗时更长 |
| 方形 1:1 | `512×512` | 0.262 | 通用尺寸，H3 更占显存 |

MP 按 `宽 × 高 ÷ 1,000,000` 计算。对 8 GB 显存设备，优先使用 `640×352`；需要更高质量时再尝试 `736×416` 或 `864×480`。
